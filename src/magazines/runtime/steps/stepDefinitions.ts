...
import type { ImageAsset, ImageAspectRatio, ImageDomainLibrary, ImageSelectionCriteria } from '../../../domain/image/index.ts';
...
export function createGenerateInstagramPreviewStep(
  ...
  return stepFactory.createStep({
    ...
    async execute(context) {
      ...
      const selectedImage = requireWorkflowData<ImageAsset>(context, 'selectedImage');
      const instagramPreview = new InstagramContentService().generate({
        publishingPackage,
        magazineProfile,
        selectedImage,
        topic: options.topic
...
export function createGeneratePodcastPreviewStep(
  stepFactory: DryRunStepFactory,
  options: MagazineDryRunStepOptions
): WorkflowStep {
  return stepFactory.createStep({
    name: 'Generate Podcast Preview',
    async execute(context) {
      const magazineProfile = requireWorkflowData<MagazineProfile>(context, 'magazineProfile');
      const publishingPackage = requireWorkflowData<PublishingPackage>(context, 'publishingPackage');
      const instagramPreview = context.data.instagramPreview as InstagramContentPackage | undefined;
      const podcastPreview = new PodcastContentService().generate({
        publishingPackage,
        magazineProfile,
        instagramPreview
      });
...
export function createGeneratePublishingPackageStep(
  stepFactory: DryRunStepFactory,
  options: MagazineDryRunStepOptions
): WorkflowStep {
  return stepFactory.createStep({
...
export function createSelectImageStep(stepFactory: DryRunStepFactory, options: MagazineDryRunStepOptions): WorkflowStep {
  return stepFactory.createStep({
    name: 'Select Image',
    async execute(context) {
      const magazineProfile = requireWorkflowData<MagazineProfile>(context, 'magazineProfile');
...
export function createLoadConfigStep(stepFactory: DryRunStepFactory, options: MagazineDryRunStepOptions): WorkflowStep {
  return stepFactory.createStep({
    name: 'Load Config',
    async execute(context) {
...
export function createLoadPromptStep(stepFactory: DryRunStepFactory, options: MagazineDryRunStepOptions): WorkflowStep {
  return stepFactory.createStep({
    name: 'Load Prompt',
    async execute(context) {
      const magazineConfig = requireWorkflowData<MagazineConfig>(context, 'magazineConfig');
...
export function createResearchStep(stepFactory: DryRunStepFactory, options: MagazineDryRunStepOptions): WorkflowStep {
  return stepFactory.createStep({
    name: 'Research',
    async execute(context) {
...
export function createGenerateArticleStep(stepFactory: DryRunStepFactory, options: MagazineDryRunStepOptions): WorkflowStep {
  return stepFactory.createStep({
...
export function createGenerateSeoStep(stepFactory: DryRunStepFactory, options: MagazineDryRunStepOptions): WorkflowStep {
  return stepFactory.createStep({
    name: 'Generate SEO',
    async execute(context) {
...
export function createGenerateMonetizationPreviewStep(
  stepFactory: DryRunStepFactory,
  options: MagazineDryRunStepOptions
): WorkflowStep {
  return stepFactory.createStep({
...
export function createPublishPreviewStep(
  stepFactory: DryRunStepFactory,
  options: MagazineDryRunStepOptions,
...
export function createSchedulePreviewStep(
  stepFactory: DryRunStepFactory,
  options: MagazineDryRunStepOptions,
      ...
      if (!queueItem) {
        return {
          ...
          data: {
            ...
            auditTimeline: options.auditLog?.listEvents(),
            metricsSnapshot: options.metricsService?.snapshot('2026-07-10T10:00:00.000Z')
          }
        };
      }
...
export function createExecutionPreviewStep(
  stepFactory: DryRunStepFactory,
  options: MagazineDryRunStepOptions,
  ...
  return stepFactory.createStep({
    ...
    async execute(context) {
      ...
      const executionResult = await scheduledJobExecutor.execute({
        ...
        publishRequest: {
          publishingPackage: schedulerResult.job.metadata?.publishingPackage as PublishingPackage
            ?? requireWorkflowData<PublishingPackage>(context, 'publishingPackage'),
          destination: (schedulerResult.job.metadata?.destination as PublishingDestination | undefined)
            ?? requireWorkflowData<PublishingQueueResult>(context, 'queueResult').item!.destination,
          mode: 'preview',
          requestedAt: '2026-07-10T10:00:00.000Z',
          metadata: {
            dryRun: true,
            magazineSlug: context.magazineSlug,
            scheduledJobId: schedulerResult.job.id
          }
        }
      });
      ...
      return {
        ...
        data: {
          ...context.data,
          executionResult,
          publisherResult: executionResult.value,
