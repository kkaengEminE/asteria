...
Sprint 52 selected SQLite as the first local/dev durable adapter path and PostgreSQL as the production adapter target. Sprint 53 implements the SQLite operational adapter. Sprint 55 adds the initial PostgreSQL operational adapter boundary for Queue, Scheduler, Job Execution, Idempotency, Locks, and UnitOfWork. Sprint 56 adds the concrete `pg` connection/pool adapter. Sprint 57 adds opt-in real PostgreSQL validation through a disposable Docker database. In-memory remains the default; SQLite is selected only when `ASTERIA_PERSISTENCE_MODE=sqlite` and `ASTERIA_SQLITE_DATABASE_PATH` are provided. PostgreSQL is selected only when `ASTERIA_PERSISTENCE_MODE=postgresql` and `ASTERIA_POSTGRESQL_URL` are provided.
...
The PostgreSQL adapter mirrors the proven SQLite operational scope first: Queue, Scheduler, Job Execution, Idempotency, Locks, and UnitOfWork. It does not expand into Audit, Metrics, Asset Catalog, Storage Metadata, publishing, or external scheduler execution. PostgreSQL-specific SQL, migration, serialization, lock, driver, pool, health check, shutdown, and transaction details stay under `src/providers/persistence/postgresql`.

The PostgreSQL real-database validation suite is deliberately opt-in through `npm run test:postgresql`. Normal `npm test` does not require Docker or PostgreSQL. When Docker is available, the suite runs against `postgres:16.6-alpine`; when Docker is unavailable, it reports skipped PostgreSQL integration tests without changing runtime behavior.
